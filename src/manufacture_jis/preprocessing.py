...
from manufacture_jis.schemas import HourlyDemand, ManufacturingPlan, PackRule, SplitDemand, TruckRule
...
def split_plan_to_hourly_quantities(plan: ManufacturingPlan) -> list[tuple[datetime, int]]:
    """Split a manufacturing plan into hourly integer consumption buckets.

    The final bucket absorbs any integer remainder so the generated quantities
    always sum exactly to the original plan quantity.
    """
...
def allowed_arrival_times(consumption_time: datetime, config: OptimizerConfig) -> list[datetime]:
    """Return feasible hourly arrival times for one consumption bucket."""

    arrivals: list[datetime] = []
    for offset in range(config.arrival_window_start_hours, config.arrival_window_end_hours + 1):
        arrival_time = consumption_time + timedelta(hours=offset)
        if arrival_time.hour not in config.forbidden_arrival_hours:
...
def build_hourly_demands(
    ...
    """Create supplier/location-aware hourly demands with pallet quantities."""
    ...
    for plan in plans:
        ...
        for consumption_time, bucket_qty in split_plan_to_hourly_quantities(plan):
            ...
            while remaining_bucket_qty > 0:
                ...
                arrivals = allowed_arrival_times(consumption_time, config)
                if not arrivals:
                    raise ValueError(
                ...
                hourly_demands.append(
                    HourlyDemand(
                        ...
                        pack_size=pack_size,
                        pallets=ceil_div(alloc_qty, pack_size),
...
def required_truck_upper_bound(total_pallets: int, capacity: int) -> int:
    """Return a safe per-supplier truck upper bound for CP-SAT variables."""
