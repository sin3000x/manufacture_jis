...
def _to_excel_file(source: XlsxSource) -> str | Path | BytesIO | BinaryIO:
    if isinstance(source, (str, Path, BytesIO)):
        return source
    return source


def _load_sheet_frame(source: XlsxSource, sheet_name: str) -> pd.DataFrame:
    return pd.read_excel(_to_excel_file(source), sheet_name=sheet_name)


def _load_plans(source: XlsxSource) -> list[ManufacturingPlan]:
    frame = _load_sheet_frame(source, INPUT_SHEETS["production_plan"])
...
def _load_split_demands(source: XlsxSource) -> list[SplitDemand]:
    frame = _load_sheet_frame(source, INPUT_SHEETS["split_result"])
    return [
        SplitDemand(
            item_code=str(row[0]),
            supplier=str(row[1]),
...
def _load_pack_rules(source: XlsxSource) -> list[PackRule]:
    frame = _load_sheet_frame(source, INPUT_SHEETS["package_rules"])
    return [PackRule(item_code=str(row[0]), pack_size=int(row[1])) for row in frame.itertuples(index=False)]


def _load_truck_rules(source: XlsxSource) -> list[TruckRule]:
    frame = _load_sheet_frame(source, INPUT_SHEETS["truck_rules"])
    return [
...
def schedule_dataframe_from_xlsx(source: XlsxSource) -> pd.DataFrame:
    """Read an input workbook, optimize it, and return the result as a DataFrame."""

    plans = _load_plans(source)
    split_demands = _load_split_demands(source)
    pack_rules = _load_pack_rules(source)
    truck_rules = _load_truck_rules(source)
